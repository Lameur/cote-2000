<?php // Keep encoding UTF WITHOUT BOM - do not edit in VS!

/**
 * openElement Recaptcha (use if you process POST data yourself)
 * 
 * @author openElement
 * Takes the Secret Key from either the page var files page_name(var).php or from the parameter, 
 * the Recaptcha response obtained from Google and passed by POST, 
 * then requests Google to check the validity of the response
 * 
 */
class OERecaptcha
{
	
	private $ok = false; // recaptcha validated ok
	
    /**
     * Initialize and request Google to check user response validity
     * 
     * @param string $secretKey Use if you process form POST data yourself - Secret key, needed if page configuration file is not available
     * @param string $idElementRecaptcha Needed if $secretKey is not provided - ID of OE  Recaptcha element
     * @return void
     */
	public function __construct($secretKey = null, $idElementRecaptcha = null) // if $secretKey is not specified, <page>(var).php should already be included; otherwise
	{
		if (!$secretKey) {
			global $OEConfWEGoogleRecaptcha;
			if (!isset($OEConfWEGoogleRecaptcha) || !$idElementRecaptcha) {
				return; // no parameters provided
			}
			
			$paramsRecaptchas = json_decode($OEConfWEGoogleRecaptcha);
			if (!empty($paramsRecaptchas)) {
				foreach ($paramsRecaptchas as $idRecaptcha => $paramsRecaptcha) {
					if ($idRecaptcha === $idElementRecaptcha) {
						$secretKey = $paramsRecaptcha->SecretKey;
						break;
					}
				}
			}			
		...
		if ($secretKey) {
			$this->recaptcha = new reCAPTCHA("site-key-does-not-matter", $secretKey);
			$this->recaptcha->setRemoteIp();
			
			$response = isset($_POST['g-recaptcha-response']) ? $_POST['g-recaptcha-response'] :
				       (isset($_GET[ 'g-recaptcha-response']) ? $_GET[ 'g-recaptcha-response'] : '');
			
			$this->ok = $this->recaptcha->isValid($response);
		}
	}
